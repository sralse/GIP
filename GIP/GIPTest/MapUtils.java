...
public class MapUtils extends Settings {
	...
	/**
	 * init() Makes sure we get the following items:
	 * A virtual image of the map
	 * An array representing the map tiles
	 * An array representing the boundries
	 * An map object list
	 * Extending the existing boundries
	 * */
	...
	private static List<String> getMapArray(int mapID) {
		...
		if (in == null) {System.err.println("Can't find ref: " + mapCurrent);}
		...
		try {
			...
			read.close();
			if (DEBUG) {System.out.println("Done loading map: " + mapCurrent);}
		} catch (IOException e) {
			e.printStackTrace(); 
			return null;
		}
	...
	/**
	 * getMapImage(String mapID) Makes sure we do the following
	 * Reads our map array
	 * Makes a virtual image of the array
	 * @param mapID The map ID we're loading.
	 ...
	 * @param charToImage The character map as a set of characters.
	...
	public static Image mapArrayToImage(int mapID) {
		...
		for(int i = 0; i < strTileData.length; i++){
			...
			tileData.add(s);
			tileImage.add(dynamics.loadImage(imgDir + s + imgExt));
