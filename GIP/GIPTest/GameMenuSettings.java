...
class GameMenuSettings extends Settings {
	...
	private static Image arrowLeft = dynamics.loadImage(uiImageDir + arrowL_grey);
	private static Image arrowRight = dynamics.loadImage(uiImageDir + arrowR_grey);
	private static Image arrowLeftSelected = dynamics.loadImage(uiImageDir + arrowL_blue);
	private static Image arrowRightSelected = dynamics.loadImage(uiImageDir + arrowR_blue);
	private static Image sliderLine = dynamics.scaleImageDetailed(dynamics.loadImage(uiImageDir + sliderLine_h), sliderWidth, sliderHeigth);
	private static Image sliderDot = dynamics.loadImage(uiImageDir + sliderEnd);
	private static Image sliderActive = dynamics.loadImage(uiImageDir + sliderD_blue);
	private static Image sliderInactive = dynamics.loadImage(uiImageDir + sliderD_grey);
	// Layout vars
	...
	private static void updateGraphics() {
		...
		for(int i = 0; i < 5; i++) {
			...
			if(i == choice) {
				...
				if(i == 0) s = menuLang[13];
				if(i == 1) s = menuLang[4];
				if(i == 2) s = menuLang[6];
				if(i == 3) s = menuLang[7];
				if(i == 4) s = menuLang[8];
				GlyphVector gv = localFont.createGlyphVector(frc, s);
				g.drawGlyphVector(gv, textColumnLeft, (float) (gv.getLogicalBounds().getHeight() / 2 + textSpacingX + textSpacing * i));
			} else {
		...
		if(choice == 0) {
			g.drawImage(sliderActive, arrowPos1 - screenCorrection + sliderVolume, textSpacingX - sliderActive.getHeight(null) + screenCorrection, null);
		} else {
			g.drawImage(sliderInactive, arrowPos1 - screenCorrection + sliderVolume, textSpacingX - sliderActive.getHeight(null) + screenCorrection, null);
		}
		...
		for(int i = 1; i < 5; i++) {
			boolean b;
			String s = "";
			if(i != choice) g.setColor(new Color (135,135,135));
		...
		if(DEBUG) g.drawString("CHOICE : " + choice, 10, 40);
		if(DEBUG) g.drawString("CYCLE : " + cycle, 10, 60);
		if(DEBUG) g.drawString("VOLUME : " + SETTING_VOLUME_db, 10, 80);
	...
	private static void updateChoice() {
		...
		if(timeNew - time > 20) {
			...
			if(leftPressed) {cycle--;}
			if(rightPressed) {cycle++;}
		...
		// Volume
		if(choice == 0) {
			if(cycle == 0) {cycle = SETTING_VOLUME;}
			...
			if(choice != oldChoice) {
				if((boolean) SETTINGS[choice] == true) cycle = 0;
				else cycle = 1;
			} else {
				if(cycle == 0) SETTINGS[choice] = true;
				else SETTINGS[choice] = false;
			}
